...
int main (int argc, char **argv)
{
    ...
    returnCode = SQLExecDirect(hstmt, (SQLCHAR *)"select * from table_name", SQL_NTS);
	if(returnCode != SQL_SUCCESS)
		LogDiagnostics("select", returnCode);
    ...
    char buffer[1024] = "";
    returnCode = SQLGetData(hstmt, 1, SQL_C_CHAR, buffer, 1024, NULL);
	if(returnCode != SQL_SUCCESS)
		LogDiagnostics("SQLGetData", returnCode);
