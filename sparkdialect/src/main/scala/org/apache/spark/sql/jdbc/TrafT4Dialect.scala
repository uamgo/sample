...
class TrafT4Dialect extends JdbcDialect {
  ...
  override def getCatalystType(
    ...
    sqlType match {
      ...
      case BINARY_FLOAT => Some(FloatType)
      case BINARY_DOUBLE => Some(DoubleType)
      case _ => None
    }
  }
  ...
  override def compileValue(value: Any): Any = value match {
    ...
    // in the format: {ts 'yyyy-mm-dd hh:mm:ss.f...'}.
  ...
  override def getTruncateQuery(
    ...
    cascade match {
      case Some(true) => s"PURGEDATA $table"
      case _ => s"PURGEDATA $table"
    }
  }
}
